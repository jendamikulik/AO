...
def universe_oscillator(spins, frustrated=False, zeta=0.12, alpha=0.03, beta=0.02, target_Psi=np.e * (1 + np.sqrt(5)) / 2):
    ...
    eps = 0.12
    sigma0 = 0.10
    theta = np.where(spins > 0, eps, np.pi - eps) + rng.normal(0, sigma0, size=N)
    theta %= 2 * np.pi
    ...
    print(f"{label} Oscillator: Final r = {r_vals[-1]:.3f}, Ψ = {Psi_final:.3f}, Bessel r_pred = {r_pred:.3f}")
...
def p_vs_np_phase_transition_demo(n_vars=50):
    ...
    for alpha in alphas:
        ...
        frustrated = alpha > 4.26  # Frustrated for high density
        universe_oscillator(spins, frustrated=frustrated)
        plt.figure()
        plt.hist(energies, bins=20, density=True)
        plt.title(f"Energy Landscape Histogram ({label})")
        plt.xlabel("Energy (unsat clauses)")
        plt.ylabel("Density")
        plt.grid(True)
        plt.show()
